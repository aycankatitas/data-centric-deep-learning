...
def get_ks_score(tr_probs, te_probs):
  ...
  ks_stat, p_value = ks_2samp(tr_probs,te_probs)
  score = p_value
  # ============================
  return score
...
def get_hist_score(tr_probs, te_probs, bins=10):
  ...
  tr_heights, bin_edges = np.histogram(tr_probs.numpy(), bins, density=True)
  te_heights, _ = np.histogram(te_probs.numpy(), bins, density=True)
  score = 0
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    iso_reg.fit(tr_probs.numpy(),te_probs.numpy())
